...
class SimpleAgent(BaseAgent):
    ...
    def on_quote(self, market_id: str, book_state: pd.Series):
        ...
        self.midpointnow[market_id] = (book_state['L1-BidPrice'] + book_state['L1-AskPrice']) / 2
        self.spread_market[market_id] = (book_state['L1-AskPrice'] - book_state['L1-BidPrice']) / self.midpointnow[market_id]
        ...
        if market_id not in self.midpointpre:
            self.midpointpre[market_id] = 0   #initialse
            quote_trig = 0                    #initilaize
        else:
            if self.midpointpre[market_id] == self.midpointnow[market_id]:
                quote_trig = 0
                c = 1
            else:
                self.midpointpre[market_id] = self.midpointnow[market_id]
                quote_trig = 1 #midpoint changes, we have to quote a new quote

        #we also delet one sided quote, if onyl one order side is active
        if len(self.market_interface.get_filtered_orders(market_id,status="ACTIVE")) == 1:
            quote_trig = 2   #then, he only deletes old quote, and waits for a new m
...
#############Evaluation################################
        ...
        if timestamp == timestamp_next:
            ...
            for market_id in self.market_interface.market_state_list.keys():
                ...
                n_shares[market_id] = sum(t.quantity for t in trades)
                n_bought[market_id] = sum(t.quantity for t in self.market_interface.get_filtered_trades(market_id,side="buy"))
                n_sold[market_id] = sum(t.quantity for t in self.market_interface.get_filtered_trades(market_id,side="sell"))
            ...
            n_shares_df = pd.DataFrame.from_dict(n_shares, orient='index')
            n_bought_df = pd.DataFrame.from_dict(n_bought, orient='index')
            n_sold_df = pd.DataFrame.from_dict(n_sold, orient='index')
            ...
            n_shares_df.columns = ["n_shares"]
            n_bought_df.columns = ["n_shares_bought"]
            n_sold_df.columns = ["n_shares_sold"]
            ...
            n_shares_df = n_shares_df.transpose()
            n_bought_df = n_bought_df.transpose()
            n_sold_df = n_sold_df.transpose()
            ...
            result_per_Share = result_per_Share.append([titm_df,proz_titm_df,VWAP_score_df,pnl_df,trades_df,trigger_storage_take_prof_df,
                                                        trigger_storage_stop_loss_df,volume_df,trading_costs_df,pnl_unr_df,n_shares_df,n_bought_df,n_sold_df])
            ...
            result_matrix = result_matrix.append(
                {
                    ...
                    "shares_traded": sum(t.quantity for t in self.market_interface.get_filtered_trades())
                }
                , ignore_index=True)
...
if __name__ == "__main__":
    ...
    agent = SimpleAgent(
        name="Bench_Agent_naiv_faster_50bspsltp",
    )
